...
/**
 * Кабинет склеивал `https://telegram.me/${support_username}` и ломал внешний хелпдеск:
 * SUPPORT_USERNAME принимает и `@user`, и произвольный URL, поэтому из
 * `https://help.example.com` получалось `https://telegram.me/https://help.example.com`.
...
describe('resolveSupportContact', () => {
  ...
  it('телеграм-контакт открывается через telegram-ссылку', () => {
    const target = resolveSupportContact(
      config({
        support_url: 'https://telegram.me/help',
        support_username: '@help',
        contact_is_telegram: true,
      }),
    );

    expect(target).toEqual({ kind: 'telegram', url: 'https://telegram.me/help' });
  });
  ...
  it('support_url без contact_is_telegram трактуется как телеграм', () => {
    ...
    const target = resolveSupportContact(config({ support_url: 'https://telegram.me/help' }));
