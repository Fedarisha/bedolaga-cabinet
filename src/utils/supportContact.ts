 ...
 * раньше склеивал `https://telegram.me/${support_username}` — на внешнем хелпдеске это
 * давало `https://telegram.me/https://help.example.com`, и ссылка не открывалась.
 *
...
export function resolveSupportContact(config: SupportConfig): SupportContactTarget | null {
  ...
  // Legacy-путь: голый URL здесь дал бы `telegram.me/https://…`, поэтому клеим telegram.me
  // только из настоящего юзернейма — всё URL-образное отдаём резолвить бэку.
  if (!/^[A-Za-z0-9_]{3,}$/.test(username)) return null;

  return { kind: 'telegram', url: `https://telegram.me/${username}` };
}
