// forked from design-sync lib/bundle.mjs — ONLY the esbuild `define` map in
// bundleToIife gains import.meta.env (VITE_*) so this Vite app's modules
// (api/client.ts reads import.meta.env.VITE_API_URL at load) don't crash the
// main bundle. stampHeader and the output contract (@ds-bundle header, IIFE
// global) are UNCHANGED — keep this fork in sync with upstream on re-sync.
  ...
  try {
    buildResult = await build({
      ...
      define: {
        ...
        global: 'globalThis',
        // Vite app: modules read import.meta.env.VITE_* at load time.
        'import.meta.env': JSON.stringify({
          ...
          VITE_APP_LOGO: '',
          VITE_CABINET_HOST: 'cabinet.example.com',
          VITE_PUBLIC_HOST: 'example.com',
          VITE_LEGAL_INFO: '',
