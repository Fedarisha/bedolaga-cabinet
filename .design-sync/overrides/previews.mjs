// forked from design-sync lib/previews.mjs — define import.meta.env for the
// preview bundle so this Vite app's modules (api/client.ts etc.) don't crash
// reading VITE_* env vars at load time. Only the `define` in buildPreviews differs.
  ...
  for (const c of components) {
    ...
    try {
      await build({
        ...
        global: 'globalThis',
          __DEV__: 'true',
          // This is a Vite app: modules read import.meta.env.VITE_* at load.
          // Provide plausible values so page previews don't crash on boot.
